              ...
              <div key={n} className="card p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <ul className="text-sm space-y-1">{(pc.cautions.length? pc.cautions : ["(No detectados)"]).slice(0,8).map((t,i)=>(<li key={i} className="badge warn flex items-center gap-2">{t}<button className="underline text-xs" onClick={()=>setShowSource(t)}>ver origen</button></li>))}</ul>
                  </div>
                  <div>
                    <h4 className="font-medium text-sm mb-1">Contras</h4>
                    <ul className="text-sm space-y-1">{(pc.cons.length? pc.cons : ["(No detectados)"]).slice(0,8).map((t,i)=>(<li key={i} className="badge con flex items-center gap-2">{t}<button className="underline text-xs" onClick={()=>setShowSource(t)}>ver origen</button></li>))}</ul>
                  </div>
                </div>
              </div>
            );
