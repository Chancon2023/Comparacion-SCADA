...
import { COLORS, scoreValue, prepareData, extractProsCons } from "../components/utils.js";
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {selected.map(n => {
            ...
            const pc = extractProsCons(s.features || {});
            return (
              <div key={n} className="card p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <ul className="text-sm space-y-1">{(pc.pros.length? pc.pros : ["(No detectados)"]).slice(0,8).map((t,i)=>(<li key={i} className="badge pro">{t}</li>))}</ul>
                  </div>
                  <div>
                    <h4 className="font-medium text-sm mb-1">Contras</h4>
                    <ul className="text-sm space-y-1">{(pc.cons.length? pc.cons : ["(No detectados)"]).slice(0,8).map((t,i)=>(<li key={i} className="badge con">{t}</li>))}</ul>
                  </div>
                </div>
              </div>
            );
